...
describe('useGenkitRealtime', () => {
  ...
  describe('Connection Management', () => {
    ...
    // --- Edge Case Tests for Real-Time Connection Issues ---
    ...
    it('should handle stream processing errors', async () => {
      const mockResponse = {
        ...
        body: new ReadableStream({
          start(controller) {
            controller.enqueue(new TextEncoder().encode('data: {"type":"control","data":{"status":"connected"}}\n\n'));
            // Simulate a malformed message to trigger a parse error
    ...
    it('should handle no response body from server', async () => {
      const mockResponse = {
        ok: true,
        body: null, // Simulate no response body
      };
      mockFetch.mockResolvedValueOnce(mockResponse);
    ...
    // TODO: Add tests for simulating network disconnection during stream (hard with mocks)
    // TODO: Add tests for simulating server-side disconnection (hard with mocks)
  ...
  describe('Sending Data', () => {
    ...
    it('should not send interrupt command when disconnected', () => {
      const { result } = renderHook(() =>
        useGenkitRealtime(mockApiUrl, mockSessionConfig)
      );
  ...
  // --- Edge Case Tests for Backend/Genkit Flow Errors ---

  // Note: Tests for specific backend error types (e.g., validation errors,
  // Google AI errors) are handled by the API route tests. These tests
  // verify that the hook correctly receives and processes error messages
  // sent from the API route via the SSE stream.

  // --- Edge Case Tests for Data Inconsistencies and Edge Cases ---

  it('should handle receiving an empty transcript message', async () => {
    const mockResponse = {
      ok: true,
      body: new ReadableStream({
        start(controller) {
          controller.enqueue(new TextEncoder().encode('data: {"type":"control","data":{"status":"connected"}}\n\n'));
          // Simulate an empty transcript message
          controller.enqueue(new TextEncoder().encode('data: {"type":"transcript","data":{"speaker":"ai","text":""}}\n\n'));
        }
      }),
    };
    mockFetch.mockResolvedValueOnce(mockResponse);
  ...
  it('should handle receiving a very short audio message', async () => {
    const mockResponse = {
      ok: true,
      body: new ReadableStream({
        start(controller) {
          controller.enqueue(new TextEncoder().encode('data: {"type":"control","data":{"status":"connected"}}\n\n'));
          // Simulate a very short audio message
          controller.enqueue(new TextEncoder().encode('data: {"type":"audio","data":"a"}\n\n')); // Very short base64 string
        }
      }),
    };
    mockFetch.mockResolvedValueOnce(mockResponse);
    ...
    await waitFor(() => {
      expect(result.current.aiAudioQueue).toHaveLength(1);
      expect(result.current.aiAudioQueue[0]).toBe('a');
    });
  });

  // TODO: Add tests for handling delayed audio chunks (hard with mocks)
  // TODO: Add tests for handling an empty feedback object (if feedback is received via SSE)

  describe('Cleanup', () => {
    it('should disconnect on unmount (simulating navigation)', async () => {
      mockFetch.mockResolvedValueOnce({ ok: true });
      ...
      // Simulate navigating away by unmounting the hook
      unmount();
    ...
    // Note: Testing closing the browser tab requires end-to-end tests.
  });
});
